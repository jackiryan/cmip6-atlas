...
def time_subset(ds: xr.Dataset, start_date: datetime, end_date: datetime) -> xr.Dataset:
    """
    ...
    if isinstance(time_sample, np.datetime64):
        ...
        return ds.sel(time=slice(np.datetime64(start_date), np.datetime64(end_date)))
